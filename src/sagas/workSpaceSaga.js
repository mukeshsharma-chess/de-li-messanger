...
import { ADD_NEW_CHATTING_WITH_CHANNEL_DATA_FAILED, ADD_NEW_CHATTING_WITH_CHANNEL_DATA_RESPONSE, CHATTING_WITH_CHANNEL_DATA_RESPONSE, DELETE_CHATTING_WITH_CHANNEL_DATA_FAILED, DELETE_CHATTING_WITH_CHANNEL_DATA_RESPONSE, GET_LATEST_MESSAGE_OF_CHANNEL_FAILED, GET_LATEST_MESSAGE_OF_CHANNEL_RESPONSE, MEMBER_OF_CHANNEL_FAILED, MEMBER_OF_CHANNEL_RESPONSE } from '@/redux/types/chattingWithChannelType';
...
export function* getLatestMessagesChannelSaga({ payload, resolve }) {
    try {
        ...
        if (status === 200) {
            yield put({ type: GET_LATEST_MESSAGE_OF_CHANNEL_RESPONSE, payload: response.data })
            resolve && resolve(response)
            yield put({ type: RESET_LOADER, isLoading: false })
        }
        else {
...
export function* sendMessageToChannelSaga({ payload, resolve }) {
    try {
        yield put({ type: START_LOADING, isLoading: true })
        let response = yield api.SendMessageToChannel(payload);

        // const {data, status} = response;

        // if (status === 200) {
            yield put({ type: ADD_NEW_CHATTING_WITH_CHANNEL_DATA_RESPONSE, payload: response })
            resolve && resolve(response)
            yield put({ type: RESET_LOADER, isLoading: false })
        // }
        // else {
        //     console.log("ADD_NEW_CHATTING_WITH_CHANNEL_DATA_FAILED", data);
        //     yield put({ type: ADD_NEW_CHATTING_WITH_CHANNEL_DATA_FAILED, payload: data })
        //     resolve && resolve(response)
        //     yield put({ type: RESET_LOADER, isLoading: false })
        // }
    } catch (e) {
        yield put({ type: ADD_NEW_CHATTING_WITH_CHANNEL_DATA_FAILED, payload: e })

    }
}

export function* deleteMessageToChannelSaga({ payload, resolve }) {
    try {
        yield put({ type: START_LOADING, isLoading: true })
        let response = yield api.DeleteMessageToChannel(payload);

        // const {data, status} = response;

        // if (status === 200) {
            yield put({ type: DELETE_CHATTING_WITH_CHANNEL_DATA_RESPONSE, payload: response })
            resolve && resolve(response)
            yield put({ type: RESET_LOADER, isLoading: false })
        // }
        // else {
        //     console.log("DELETE_CHATTING_WITH_CHANNEL_DATA_FAILED", data);
        //     yield put({ type: DELETE_CHATTING_WITH_CHANNEL_DATA_FAILED, payload: data })
        //     resolve && resolve(response)
        //     yield put({ type: RESET_LOADER, isLoading: false })
        // }
    } catch (e) {
        yield put({ type: DELETE_CHATTING_WITH_CHANNEL_DATA_FAILED, payload: e })
